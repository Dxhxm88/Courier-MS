<?php
		...
		if(isset($_GET["action"])){
			if($_GET["action"] == "delete"){
				...
				//make the query 
				$q = "DELETE FROM kuptmparcelinfo WHERE id = $id LIMIT 1";
		...
		if(isset($_POST["search"])){
			if($_POST['searchParcel'] == "collected"){
				$query1 = "SELECT * FROM kuptmparcelinfo WHERE status = 'COLLECTED'";
			}
			elseif($_POST['searchParcel'] == "uncollected"){
				$query1 = "SELECT * FROM kuptmparcelinfo WHERE status = 'UNCOLLECTED'";
			}
			else{
				$query1 = "SELECT * FROM kuptmparcelinfo ORDER BY id ASC";
			}
...
<body>
	<div class="main">
        ...
        <div class="search-parcel" style="text-align:right">
        	<div class="container">
            <br />
            	<form method="post" action="kuptmViewParcel.php">
                	<label>Filter</label>                 
      					<select name="searchParcel" id="filter">
                    ...
                    <table>
					   ...
					   <?php
                            ...
                            while($row = mysqli_fetch_array($result)){
                                ?>
                                    <form method="post" action="kuptmViewParcel.php?action=add&id=<?php echo $row["id"]; ?>">
                                    ...
                                    <tr>                        	
                                        ...
                                        <td><a href="kuptmViewParcel.php?action=delete&id=<?php echo $row["id"];?>" onclick="return confirm('Are sure to delete data?')">
                                        <i class='far fa-trash-alt' style='font-size:18px'></i></a></td>
                                        
                                        <td><a href="kuptmEditparcel.php?action=edit&id=<?php echo $row['id']; ?>">
                                        <i class='fas fa-cog' style='font-size:18px'></i></a></td>
