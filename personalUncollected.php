...
<body>
	<div class="main">
        ...
        <table class = "table ">
           ...
           <?php
		   		$query = "SELECT * FROM parcelinformation WHERE statusParcel = 'UNCOLLECTED' ";
				$result = mysqli_query($connect, $query);
				if(mysqli_num_rows($result) > 0){
				
				while($row = mysqli_fetch_array($result)){
					?>
                        <form method="post" action="personalUncollected.php?action=add&id=<?php echo $row["id"]; ?>">
						...
						<tr>                        	
							...
							<td><a href="personalUncollected.php?action=delete&id=<?php echo $row["id"]; ?>"><i class='far fa-trash-alt' style='font-size:18px'></i></a></td>
						</tr> 
